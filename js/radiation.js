...
// Reduced from 5000 → 500 to avoid Worker CPU/memory limit (was causing 530 errors)
const EURDEP_URL = "https://rmap.jrc.ec.europa.eu/rmap/api/v1/gamma?format=json&valid=true&limit=500";
...
function _radColor(u) {
  return u >= 5 ? "#ff0000" : u >= 1 ? "#ff6600" : u >= 0.3 ? "#ffcc00" : "#00cc44";
}

async function _fetchEURDEP() {
  try {
    const r = await fetch(PROXY(EURDEP_URL), { signal: AbortSignal.timeout(20000) });
    ...
    return arr.map(s => {
      ...
      const usv = parseFloat(s.value || s.doseRate || 0);
      if (isNaN(lat) || isNaN(lon) || isNaN(usv)) return null;
      return {
        type: "Feature",
        geometry: { type: "Point", coordinates: [lon, lat] },
        properties: {
          usv, source: "EURDEP",
          id: String(s.stationId || s.id || ""),
          time: String(s.measurementTime || ""),
          color: _radColor(usv),
          elevated: usv >= 0.3
        }
      };
    }).filter(Boolean);
  } catch(e) { console.warn("[WWO] EURDEP:", e.message); return []; }
}

async function _fetchRadNet() {
  try {
    const r = await fetch(PROXY(RADNET_URL), { signal: AbortSignal.timeout(15000) });
    if (!r.ok) throw new Error(r.status);
    const data = await r.json();
    const st = Array.isArray(data) ? data : (data.stations || data.data || []);
    return st.map(s => {
      const lat = parseFloat(s.latitude || s.lat);
      const lon = parseFloat(s.longitude || s.lon);
      let usv = parseFloat(s.gamma_dose_rate || s.doseRate || s.value || 0);
      if (s.unit && s.unit.toLowerCase().includes("r/h") && !s.unit.toLowerCase().includes("sv")) usv *= 0.00877;
      ...
      return {
        ...
        properties: {
          ...
          city: String(s.city || s.location || ""),
          state: String(s.state || ""),
          time: String(s.sample_date || ""),
          color: _radColor(usv),
          elevated: usv >= 0.3
        }
      };
    }).filter(Boolean);
  } catch(e) { console.warn("[WWO] RadNet:", e.message); return []; }
}

async function _fetchNRA() {
  try {
    const r = await fetch(PROXY(NRA_URL), { signal: AbortSignal.timeout(15000) });
    if (!r.ok) throw new Error(r.status);
    const data = await r.json();
    const st = Array.isArray(data) ? data : (data.stations || data.data || []);
    return st.map(s => {
  ...
  } catch(e) { console.warn("[WWO] NRA:", e.message); return []; }
}
