...
const TELEGRAM_OSINT_CHANNELS=[
  // PRIMARY: Verified English-language OSINT channels
  {channel:'OSINTdefender',     label:'OSINT Defender',       zone:'MULTI',   ty:'al', twitter:'sentdefender'},
  {channel:'osinttechnical',    label:'OSINT Technical',      zone:'Ukraine', ty:'al', twitter:'osinttechnical'},
  // UKRAINE (English-language only)
  {channel:'TheDeadDistrict',   label:'Dead District',        zone:'Ukraine', ty:'al', twitter:null},
  {channel:'wartranslated',     label:'WarTranslated',        zone:'Ukraine', ty:'al', twitter:'wartranslated'},
  {channel:'NOELreports',       label:'NOEL Reports',         zone:'Ukraine', ty:'al', twitter:'NOELreports'},
  // MIDDLE EAST (English-language only)
  {channel:'TheIntelLab',       label:'Intel Lab',            zone:'MENA',    ty:'wa', twitter:'TheIntelLab'},
  {channel:'IranIntl_En',       label:'Iran Intl English',    zone:'Iran',    ty:'wa', twitter:'IranIntl_En'},
  {channel:'ELINTNews',         label:'ELINT News',           zone:'MULTI',   ty:'al', twitter:'ELINTNews'},
  // GLOBAL / MULTI-THEATER
  {channel:'inteaboratory',     label:'Intel Repository',     zone:'MULTI',   ty:'al', twitter:null},
  {channel:'geaboratory',       label:'GeoConfirmed',         zone:'MULTI',   ty:'in', twitter:'GeoConfirmed'},
  // AFRICA (English)
  {channel:'africanosint',      label:'Africa OSINT',         zone:'Sudan',   ty:'al', twitter:null},
];

// Master store: all real posts pulled from Telegram, used by both OSINT feed AND conflict trackers
const liveTelegramPosts=[];
const MAX_TG_POSTS=200;
...
function parseTelegramHTML(html,ch){
  const msgs=[];
  // Telegram web preview uses .tgme_widget_message_text for message content
  // and data-post="channel/id" for post IDs
    ...
    try{
      ...
      if(msgs.length>0){
        ...
        msgs.slice(-15).forEach(msg=>{
          ...
          const postObj={text:msg.text,source:ch.label,pubDate:pubDate.toISOString(),link,zone:ch.zone,ty:ch.ty,channel:ch.channel};
          liveTelegramPosts.push(postObj);
          if(liveTelegramPosts.length>MAX_TG_POSTS)liveTelegramPosts.shift();
          addLiveItem(msg.text,ch.label,pubDate.toISOString(),link,ch.zone,ch.ty,true);
          added++;
        });
...
function injectTgMessages(messages,ch,cutoff){
  ...
  messages.forEach(msg=>{
    ...
    if(pubDate.getTime()<cutoff)return;
    const postId=msg.id||'';
    // Dedup: skip if we've already seen this post
    ...
    const postObj={text,source:ch.label,pubDate:pubDate.toISOString(),link,zone:ch.zone,ty:ch.ty,channel:ch.channel};
    liveTelegramPosts.push(postObj);
    if(liveTelegramPosts.length>MAX_TG_POSTS)liveTelegramPosts.shift();
    addLiveItem(text,ch.label,pubDate.toISOString(),link,ch.zone,ch.ty,true);
    added++;
  });
  return added>0;
}
