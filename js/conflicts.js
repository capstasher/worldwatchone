...
// ── Fetch one boundary via Worker → Overpass OSM ─────────────────────────────
// key: either an ISO-2 code (e.g. 'IR') or 'rel:12345678' for an OSM relation ID
async function _fetchBoundary(key) {
  if (_boundaryCache[key]) return;
  try {
    const param = key.startsWith('rel:')
      ? `rel=${key.slice(4)}`
      : `iso=${key}`;
    ...
    if (geom) { _boundaryCache[key] = geom; console.log('[WWO] Boundary cached:', key); }
    else console.warn('[WWO] No geometry for', key);
  } catch(e) { console.warn('[WWO] Boundary failed:', key, e.message); }
}
